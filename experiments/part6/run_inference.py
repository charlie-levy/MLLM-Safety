#!/usr/bin/env python
"""
run_inference.py — Part 6: re-run of Part 5 at LOWER corruption severities.

IDENTICAL to Part 5 (experiments/part5/run_inference.py) in every respect —
same two TIS checkpoints, same SIUO samples, same corruption_lib, same PEFT
adapter load, same frozen greedy 2048-tok generation, same output schema and
per-idx resume — EXCEPT the corruption severity:

                 Part 5 (severity_for)      Part 6 (this file)
    zoom_blur          3                          2
    glass_blur         5                          3

The ONLY functional change vs Part 5 is the PART6_SEVERITY map below; the
corruption_lib.severity_for() defaults are left untouched (shared with Parts
1/3/4/5). Everything else is byte-for-byte the Part-5 pipeline so the two runs
are directly comparable at different severities.
...
used in Part 5. Generation REUSES run_eval.generate_one UNCHANGED.
...
# Part 6 severities — the ONLY functional change vs Part 5. Part 5 used
# corruption_lib.severity_for() (zoom_blur=3, glass_blur=5); Part 6 lowers them.
# Any condition not listed falls back to severity_for() so behaviour is otherwise
# byte-identical to Part 5.
PART6_SEVERITY = {"zoom_blur": 2, "glass_blur": 3}
...
def main():
    ...
    ap.add_argument("--output_dir", default="/home/ch169788/experiments/part6/results")
    ...
    sev = 0 if is_clean else PART6_SEVERITY.get(args.condition, severity_for(args.condition))
    ...
    print("=" * 80, flush=True)
    print("  Part6 INFER | siuo cond=%s(sev%d) model=%s | adapter=%s | %d samples%s  [NO JUDGE]"
    ...
    # run_eval's frozen greedy 2048-tok generation (identical to Part-5).
