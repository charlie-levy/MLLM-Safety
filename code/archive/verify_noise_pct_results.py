#!/usr/bin/env python3
"""
verify_noise_pct_results.py — Validate the percentage-NOISE sweep for TIS/SAGE/MSR.

Checks each cell (model x metric x {0,20,40,60,80,100}%) for correct model tag,
...
Run ON NEWTON from repo root (reads ./results/). Usage:
  python3 code/verify_noise_pct_results.py
"""
import os, json
...
def asr_paths(tag, p):
    return (f"{R}/figstep_noise_sweep/asr_{tag}_clean.json" if p == 0
            else f"{R}/figstep_noise_pct/asr_{tag}_gaussian_noise_pct_p{p}.json")

def orr_paths(tag, p):
    return (f"{R}/orr_{tag}/orr_results.json" if p == 0
            else f"{R}/orr_noise_pct/orr_{tag}_gaussian_noise_pct_p{p}.json")

def sqa_paths(tag, p):
    return (f"{R}/sqa_noise_sweep/judged_{tag}_clean.json" if p == 0
            else f"{R}/sqa_noise_pct/judged_{tag}_gaussian_noise_pct_p{p}.json")
