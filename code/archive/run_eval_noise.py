# -*- coding: utf-8 -*-
"""
run_eval_noise.py — BASE + TIS model evaluation on FigStep with noise (severity=3)
"""
...
def save_sample_images(samples: list[dict], model_tag: str, num_samples: int = 5) -> None:
    """Save 5 random before/after noise images for visual verification."""
    try:
        from imagecorruptions import corrupt

        results_dir = os.path.join(RESULTS_DIR, f"{model_tag}_noise")
        ...
        for i, sample in enumerate(selected):
            ...
            # Apply noise
            noised_array = corrupt(original_array, corruption_name="gaussian_noise", severity=3)
            noised_img = Image.fromarray(noised_array)

            # Save side-by-side
            combined_width = original_img.width + noised_img.width + 10
            combined = Image.new("RGB", (combined_width, original_img.height), color=(255, 255, 255))
            combined.paste(original_img, (0, 0))
            combined.paste(noised_img, (original_img.width + 10, 0))
...
def save_results(tag: str, model_tag: str, results: list[dict], metrics: dict) -> str:
    """Save results to results/<model_tag>_noise/ directory."""
    results_dir = os.path.join(RESULTS_DIR, f"{model_tag}_noise")
...
def main():
    ...
    print("=" * 70)
    print("  ✓ Model:      BASE + TIS")
    print("  ✓ Dataset:    FigStep only (500 samples)")
    print("  ✓ Corruption: gaussian_noise, severity=3")
    ...
    # Create evaluator with gaussian_noise corruption
    evaluator = Evaluator(model, processor, corruption_type="gaussian_noise", corruption_severity=3)
    ...
    print("\n" + "=" * 70)
    print("  STEP 2.5 — Saving Sample Images (before/after noise)")
    print("=" * 70)
    ...
    print("=" * 70)
    print(f"\n--- FigStep ({len(figstep_samples)} samples) with gaussian_noise severity=3 ---")
    ...
    print("=" * 70)
    save_results("figstep_noise_severity3", model_tag, figstep_results, figstep_metrics)

    print(f"\nResults saved to: {os.path.join(RESULTS_DIR, f'{model_tag}_noise')}/")
