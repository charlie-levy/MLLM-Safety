...
\paragraph{Judge validity.} Because every headline number flows through the GPT-4o judge, we validate it against human labels. We draw a stratified sample of \TODO{$N$} SIUO responses balanced across clean/corrupted conditions and across the judge's harmful/safe verdicts, and one author labels each response's final answer as harmful or safe blind to the judge's decision. We report Cohen's $\kappa$ between judge and human on this sample \TODO{($\kappa{=}\ldots$)}; agreement is \TODO{high/moderate}, and the disagreements are concentrated in \TODO{borderline cases}. This audit bounds the judge noise that the paired statistics in \cref{sec:results} otherwise assume is unbiased.
