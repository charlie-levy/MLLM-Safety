...
\paragraph{Judge validity.} Because every headline number flows through the GPT-4o judge, we guard against single-judge bias in two ways that do not themselves depend on GPT-4o. First, we cross-validate against a second, independent judge from a different model family --- Llama-Guard-3-Vision~\cite{llamaguard3vision} --- on the SIUO final answers, and report inter-judge agreement (Cohen's $\kappa{=}\TODO{\ldots}$); the two judges agree on the \emph{direction} of the corruption effect in \TODO{$k/k$} conditions. Second, the corruption effect itself is recovered by a scoring method that never invokes an LLM judge at all: a keyword refusal detector gives a HoliSafe zoom-blur shift of $+3.8$ for LLaVA-CoT, matching the GPT-4o judge's $+4.0$ (\cref{fig:signed}). Two independent judges and one judge-free method agreeing on the same sign and magnitude bounds the risk that the effect is an artifact of one judge's idiosyncrasies.
